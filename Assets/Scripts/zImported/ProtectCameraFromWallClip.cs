...
namespace UnityStandardAssets.Cameras
{
    public class ProtectCameraFromWallClip : MonoBehaviour
    {
        ...
        private void LateUpdate()
        {
            ...
            m_Ray.direction = -m_Pivot.forward; // (m_Pivot.position - m_Cam.position).normalized;//-m_Pivot.forward;
            ...
            {
                if ((!cols[i].isTrigger) && !(cols[i].attachedRigidbody != null && cols[i].attachedRigidbody.CompareTag(dontClipTag)) && !cols[i].CompareTag("Player"))
                {
            ...
            for (int i = 0; i < m_Hits.Length; i++)
            {
                ...
                if (m_Hits[i].distance < nearest && (!m_Hits[i].collider.isTrigger) &&
                    !(m_Hits[i].collider.attachedRigidbody != null && m_Hits[i].collider.attachedRigidbody.CompareTag(dontClipTag)) && !m_Hits[i].collider.CompareTag("Player"))
                {
